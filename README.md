...
- **Organized inside the merged compendiums** — "Merged Items" is sorted into in-compendium folders (Weapons, Armor, Equipment, Consumables, Tools, Trade Goods, Treasure, Loot, Containers — "Armor" is split out of general dnd5e "Equipment" items automatically; "Treasure" — art objects and gemstones — and "Trade Goods" — raw tradeable commodities — are both split out of general "Loot"), each of which (except Trade Goods, Treasure, and Loot) also gets a rarity sub-folder (Mundane, Common, Uncommon, Rare, Very Rare, Legendary, Artifact). "Merged Spells" is sorted into a folder per spell level (Cantrips, 1st Level, ... 9th Level), "Merged Monsters" into a folder per Challenge Rating **or** creature type (Aberration, Beast, Humanoid, ...) — your choice, via a toggle in the Monsters section — "Merged Feats" into Feats, Supernatural Gifts (Epic Boons), and Enchantments (Artificer Infusions) folders, "Merged Classes" into Classes, Subclasses, and Class Features folders (the last holding every mechanical passive/feature a class or subclass grants, kept separate from the two container items), "Merged Backgrounds" into Backgrounds and Background Features folders (same idea — the passive ability a background grants, like "Shelter of the Faithful," is kept separate from the background itself), and "Merged Species" into Species and Species Traits folders (same idea again — a racial trait like "Draconic Ancestry" is kept separate from the species itself). "Merged Vehicles" isn't sorted into sub-folders yet — it's a flat, alphabetical list.
